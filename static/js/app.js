...
function updateFilters() {
  ...
  function filterTable() {
    ...
    let listChanges = Object.keys(filters);
     for (let i = 0; i < listChanges.length; i++) {
    //  filteredData = filteredData.filter(row => row.state === filters[listChanges[i]] );
     filteredData = filteredData.filter(row => eval("row."+listChanges[i]) === filters[listChanges[i]] );
    }
